...
1. **修改密码功能**
   ...
   - 新密码最少6位验证
   - 密码修改成功后自动退出登录
   - API端点：`POST /api/auth/password`

2. **用户信息组件统一**
   - 新增 `UserAvatar.vue` 公共组件
   - 统一所有页面的用户信息展示
   - 鼠标hover显示下拉菜单
   - 菜单包含：个人中心、更换头像、AI配置、退出登录
   - 避免代码重复，便于后续维护
...
### 1. 客户端AI调用 (P1) ⭐ 新增
- [ ] **客户端直接调用AI**: 前端直接调用OpenAI/Claude等API，无需后端转发
- [ ] **降低延迟**: 减少服务器中转，提升响应速度
- [ ] **减轻服务端压力**: 后端不再处理AI API调用
- [ ] **回退机制**: CORS不支持时自动回退到后端调用
- [ ] **新增服务**: `aiClient.ts` 客户端AI调用服务
...
- ✅ **减轻服务端压力**: 后端不再处理AI API调用
- ✅ **回退机制**: CORS不支持时自动回退到后端调用
- ✅ **项目上下文支持**: 客户端调用包含完整项目结构、依赖解析
- ✅ **智能文件解析**: 自动从AI响应提取多文件内容

**技术变更**
- 新增 `src/services/aiClient.ts` 客户端AI调用服务
- 支持 OpenAI、Anthropic Claude、智谱AI、通义千问 等提供商
- AIAssistant.vue 支持客户端/后端智能切换
- 后端 `SmartCodeGeneratorService` 提示词完全兼容前端
